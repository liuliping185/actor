...
function getCameraTeam(data){
    $("#membername").html("发布人: " + data.membername);
    if(data.camerateaminfo.price && data.camerateaminfo.unit){
        $("#price").html("￥" +  data.camerateaminfo.price + "/" + data.camerateaminfo.unit);
    }

    $("#role").html("找摄影组");

    infos += "<h4 style='margin-top:3%; margin-left: 5%; color:#7B7B7B;'><span style='margin-top:3%;'>摄影组信息</span></h4>";
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>";
    if(data.camerateaminfo.provience && data.camerateaminfo.city && data.camerateaminfo.district){
        infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:13px;color:#8E8E8E;'>" + data.camerateaminfo.provience + data.camerateaminfo.city + data.camerateaminfo.district + data.camerateaminfo.address + "</span>";
    }
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>" + data.camerateaminfo.camerateaminfos + "</span>";
    infos += "</span>";
    ...
    keywords += "<span style='font-size:12px;padding-left:5px;padding-right:5px;'>" + data.camerateaminfo.keywords + "</span>";
    keywords += "</span>";
    keywords += "<div>";
    ...
    // workingRange += "<div>";
    // workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.camerateaminfo.camerateaminfos + "</span>";
    // workingRange += "</div>";

    workingRange += "<h4 style='margin-left:5%; margin-top:0%; color:#7B7B7B;'><span style='margin-top:3%;'>工作范围</span><h4>";
    workingRange += "<div>";
    if(data.camerateaminfo.workarea){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.camerateaminfo.workarea + "</span>";
    }else{
    ...
    workingRange += "<div>";
    if(data.camerateaminfo.experience){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.camerateaminfo.experience + "</span>";
    }else{
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>暂无</span>";
    }
    ...
    $("#infoid").val(data.camerateaminfo.id);
    $("#type").val("camerateam");
    $("#ownerid").val(data.camerateaminfo.memberid);
    $("#nickname").html(data.camerateaminfo.camerateamname);
    if(0 < data.fiveOrder.length){
        $("#reserveTime").html(reserveTime);
    }

    var d = new Date(data.camerateaminfo.createtime);
    ...
    $('#imgBig').attr('src', data.camerateaminfo.firstimg);
