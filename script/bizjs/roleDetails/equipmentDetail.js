...
function getEquipment(data){
    ...
    infos += "<h4 style='margin-top:3%; margin-left: 5%; color:#7B7B7B;'><span style='margin-top:3%;'>设备信息</span></h4>";
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>";
    if(data.equipmentinfo.provience && data.equipmentinfo.city && data.equipmentinfo.district){
        infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:13px;color:#8E8E8E;'>" + data.equipmentinfo.provience + data.equipmentinfo.city + data.equipmentinfo.district + data.equipmentinfo.address + "</span>";
    }
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>" + data.equipmentinfo.equipmentinfos + "</span>";
    infos += "</span>";
    ...
    keywords += "<span style='font-size:12px;padding-left:5px;padding-right:5px;'>" + data.equipmentinfo.keywords + "</span>";
    keywords += "</span>";
    keywords += "<div>";
    ...
    // workingRange += "<div>";
    // workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.equipmentinfo.equipmentinfos + "</span>";
    // workingRange += "</div>";

    workingRange += "<h4 style='margin-left:5%; margin-top:0%; color:#7B7B7B;'><span style='margin-top:3%;'>工作范围</span><h4>";
    workingRange += "<div>";
    if(data.equipmentinfo.workarea){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.equipmentinfo.workarea + "</span>";
    }else{
    ...
    workingRange += "<div>";
    if(data.equipmentinfo.experience){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.equipmentinfo.experience + "</span>";
    }else{
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>暂无</span>";
    }
    ...
    var d = new Date(data.equipmentinfo.createtime);
    ...
    $('#imgBig').attr('src', data.equipmentinfo.firstimg);
