...
function getInvestment(data){
    $("#membername").html("发布人: " + data.membername);
    if(data.investmentinfo.price && data.investmentinfo.unit){
        $("#price").html("￥" +  data.investmentinfo.price + "/" + data.investmentinfo.unit);
    }

    $("#role").html("找投资");

    infos += "<h4 style='margin-top:3%; margin-left: 5%; color:#7B7B7B;'><span style='margin-top:3%;'>投资信息</span></h4>";
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>";
    if(data.investmentinfo.provience && data.investmentinfo.city && data.investmentinfo.district){
        infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:13px;color:#8E8E8E;'>" + data.investmentinfo.provience + data.investmentinfo.city + data.investmentinfo.district + data.investmentinfo.address + "</span>";
    }
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>" + data.investmentinfo.investmentinfos + "</span>";
    infos += "</span>";
    ...
    keywords += "<span style='font-size:12px;padding-left:5px;padding-right:5px;'>" + data.investmentinfo.keywords + "</span>";
    keywords += "</span>";
    keywords += "<div>";
    ...
    // workingRange += "<div>";
    // workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.investmentinfo.investmentinfos + "</span>";
    // workingRange += "</div>";

    workingRange += "<h4 style='margin-left:5%; margin-top:0%; color:#7B7B7B;'><span style='margin-top:3%;'>工作范围</span><h4>";
    workingRange += "<div>";
    if(data.investmentinfo.workarea){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.investmentinfo.workarea + "</span>";
    }else{
    ...
    workingRange += "<div>";
    if(data.investmentinfo.experience){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.investmentinfo.experience + "</span>";
    }else{
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>暂无</span>";
    }
    ...
    $("#infoid").val(data.investmentinfo.id);
    $("#type").val("investment");
    $("#ownerid").val(data.investmentinfo.memberid);
    $("#nickname").html(data.investmentinfo.nickname);
    if(0 < data.fiveOrder.length){
        $("#reserveTime").html(reserveTime);
    }

    var d = new Date(data.investmentinfo.createtime);
    ...
    $('#imgBig').attr('src', data.investmentinfo.firstimg);
