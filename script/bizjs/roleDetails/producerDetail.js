...
function getProducer(data){
    ...
    if(data.producerinfo.price && data.producerinfo.unit){
        $("#price").html("￥" +  data.producerinfo.price + "/" + data.producerinfo.unit);
    }

    $("#role").html("找制片");

    infos += "<h4 style='margin-top:3%; margin-left: 5%; color:#7B7B7B;'><span style='margin-top:3%;'>制片信息</span></h4>";
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>";
    if(data.producerinfo.provience && data.producerinfo.city && data.producerinfo.district){
        infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:13px;color:#8E8E8E;'>" + data.producerinfo.provience + data.producerinfo.city + data.producerinfo.district + data.producerinfo.address + "</span>";
    }
    infos += "<span style='margin-top: 2%; margin 0 auto; margin-left:5%; font-size:12px;color:#9D9D9D;padding-bottom:5px;'>" + data.producerinfo.producerinfos + "</span>";
    infos += "</span>";
    ...
    keywords += "<span style='font-size:12px;padding-left:5px;padding-right:5px;'>" + data.producerinfo.keywords + "</span>";
    keywords += "</span>";
    keywords += "<div>";
    ...
    // workingRange += "<div>";
    // workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.producerinfo.producerinfos + "</span>";
    // workingRange += "</div>";

    workingRange += "<h4 style='margin-left:5%; margin-top:0%; color:#7B7B7B;'><span style='margin-top:3%;'>工作范围</span><h4>";
    workingRange += "<div>";
    if(data.producerinfo.workarea){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.producerinfo.workarea + "</span>";
    }else{
    ...
    workingRange += "<div>";
    if(data.producerinfo.experience){
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>" + data.producerinfo.experience + "</span>";
    }else{
        workingRange += "<span style='margin-left:10%;font-size:14px;color:#9D9D9D'>暂无</span>";
    }
    ...
    var d = new Date(data.producerinfo.createtime);
    ...
    $('#imgBig').attr('src', data.producerinfo.firstimg);
